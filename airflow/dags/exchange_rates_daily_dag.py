...
# Добавляем путь к проекту, чтобы Airflow видел модули src
PROJECT_PATH = os.getenv("PROJECT_PATH", "/opt/airflow/project")
if PROJECT_PATH not in sys.path:
    sys.path.insert(0, PROJECT_PATH)

# Базовые настройки задач DAG
DEFAULT_ARGS = {
    "owner": "airflow",
...
# Обновляет курсы валют на дату запуска DAG и сохраняет результат в XCom

...
CREATE TABLE IF NOT EXISTS jobs_curated (
   ...
   -- Зарплата до валютной нормализации
    salary_from INTEGER,
    salary_to INTEGER,
    currency TEXT,
    salary_period TEXT,
    ...
    years_experience_min NUMERIC(6,2),
    years_experience_max NUMERIC(6,2),
   ...
   -- Дополнительные признаки
    ...
    analytics_role TEXT,
    analytics_row_ok BOOLEAN DEFAULT FALSE,
    salary_text TEXT,
    salary_mid_monthly NUMERIC(14,2),
...
-- Уникальность вакансий в источнике
...
CREATE INDEX IF NOT EXISTS idx_jobs_specialty
    ON jobs_curated (specialty);

CREATE INDEX IF NOT EXISTS idx_jobs_specialty_category
    ON jobs_curated (specialty_category);

CREATE INDEX IF NOT EXISTS idx_jobs_posting_language
    ON jobs_curated (posting_language);
...
CREATE TABLE IF NOT EXISTS job_registry (
    ...
    -- История появления вакансии
    first_seen_run_id TEXT NOT NULL,
    last_seen_run_id TEXT NOT NULL,
    first_seen_at TIMESTAMP NOT NULL DEFAULT NOW(),
    last_seen_at TIMESTAMP NOT NULL DEFAULT NOW(),

    -- Последние артефакты и статус
